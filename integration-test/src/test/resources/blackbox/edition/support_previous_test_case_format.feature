...
Feature: Support previous test case format
    ...
    Scenario Outline: Save or Update a test case (<scenario>) using raw edition with an old format
        ...
        Then the persisted test case is converted to the last format
            Do sql Request scenario version from Chutney database
                On CHUTNEY_DB
                With statements
                | SELECT VERSION FROM SCENARIO WHERE ID = '${#scenarioId}' and ACTIVATED = TRUE |
    ...
    Scenario Outline: Consult an existing old format scenario (<scenario>) using GWT form view
        Given an existing <scenario> written in format <version>
            ...
            Do sql
                On CHUTNEY_DB
                With statements
                | INSERT INTO SCENARIO (VERSION, ID, TITLE, DESCRIPTION, CONTENT, TAGS, CREATION_DATE, DATASET, ACTIVATED) VALUES ('<version>', ${#scenarioId}, 'already existing scenario', null, '${#escapeSql(#str_replace(#resourceContent("blackbox/raw_scenarios/<scenario>", null), "\\R", " "))}', null, CURRENT_TIMESTAMP(6), '{}', TRUE) |
        ...
        Examples:
            | scenario                          | version | givens | when_sentence | thens |
    ...
    Scenario Outline: Execute existing scenario (<scenario>) containing errors
        ...
        Examples:
            | version | scenario                          | message                                                                                                                | env_id |
